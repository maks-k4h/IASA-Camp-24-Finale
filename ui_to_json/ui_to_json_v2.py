...
def init_gui_model(rlobloflow_api_key: str) -> None:
    global gui_model
    gui_model = get_roboflow_model(model_id="tommorow-never-dies/3", api_key=rlobloflow_api_key)
...
def get_bboxes(image_p: Path) -> List[Tuple[str, Tuple[int, int, int, int]]]:
    ...
    image = fancy_control_buttons_remover(image)

    results = gui_model.infer(image, confidence=0.02, overlap=0.6)
    detections = sv.Detections.from_inference(results[0].dict(by_alias=True, exclude_none=True))
    if 'class_name' in detections.data:
        res = [(cls, (x1, y1, x2, y2)) for cls, (x1, y1, x2, y2) in zip(detections.data['class_name'], detections.xyxy)]
    else:
        res = []
...
def process_recursively(data_p: Path, output_p: Path, folder_hierarchy: List[str] = []) -> None:
    ...
    for sub in current_path.iterdir():
        if sub.is_file() and sub.suffix in IMAGE_SUFFS:
            ...
            json_out_p = dir_out_p / 'my_elemenets_v2.json'
...
def run(
    ...
    init_gui_model(roboflow_api_key)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rb-api-key', action='store', required=False,
                        help='roboflow api key. Needed to download the model.')
